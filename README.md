...
## Description  
* **회원가입**
	 * 서비스를 이용하기 위해 회원가입을 할 수 있어야 합니다. 사용자는 회원가입시 시터 회원으로 가입할 수도 있고, 부모 회원으로도 가입할 수 있습니다. 회원가입시 제출 받아야 할 구체적인 정보는 아래의 시터회원 / 부모회원 정보 항목을 참고하세요.
 * **로그인**
...
```
docker-compose를 실행해도 DB가 생성되지 않는다면 아래를 실행하면 됩니다.

docker exec -ti mysql57 bash
mysql --protocol=tcp -hlocalhost -P3306 -uroot -proot
mysql> CREATE DATABASE momsitter DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
...
```
./gradlew certificate-domain:flywayMigrate
```